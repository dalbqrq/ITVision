...
function make_cfg_file()
	...
	appl_cfg, rel = make_appl_cfg_file(ics)
...
function make_host_cfg_file()
  ...
  address                ]].. e[6] ..[[
				...
				if is_disableds(e[1]) then
					...
					host_cfg = host_cfg .. '  passive_checks_enabled\t\t0\n'
				end
			host_cfg = host_cfg .. '}\n\n'
			end
		end
...
function make_appl_cfg_file(ics)
	local rel = {}
	local appl = {}
	local host = {}
	local serv = {}
...
function make_serv_cfg_file()
	...
	for i,v in ipairs(serv) do
		if (v[5] ~= 'NA') and (v[5] ~= 'na') and (v[5] ~= '') and (v[6] ~= '-') then
			...
			if is_disableds(v[2]) then
				serv_cfg = serv_cfg .. '\tactive_checks_enabled\t\t0\n'
				serv_cfg = serv_cfg .. '\tpassive_checks_enabled\t\t0\n'
			end
			serv_cfg = serv_cfg .. '}\n\n\n'
		end
	end
